...
const Sidebar = ({ page }: SidebarProps) => {
  ...
  // Hide the price filter & inputs on product detail pages.
  // Matches routes like /home/product/<handle>
  const hideFilters = useMemo(() => {
    return typeof pathname === 'string' && /\/home\/product\//.test(pathname);
  }, [pathname]);
  ...
  return (
    <aside
      className="__sidebar relative left-0 top-0 flex h-auto w-20 flex-col justify-evenly bg-grayMedium p-5 py-20 text-black shadow-lg transition-all duration-300 md:min-w-[20rem] xl:min-w-[25rem]"
      aria-label="Shop sidebar"
    >
      <h2 className="mb-24 ml-4 hidden font-josefin text-4xl font-extrabold md:block xl:text-6xl">
        Explore
      </h2>
      ...
      {/* Price block: hidden on product detail pages */}
      {!hideFilters && (
      ...
      {/* Specials - keep visible even on product pages */}
      <div className="mt-6">
