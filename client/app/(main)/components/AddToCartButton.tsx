...
//           if (existsInWishlist) {
//             await dispatch(removeFromGuestWishlist(product.id)).unwrap().catch(() => {});
//           }
//         }
//       } catch (e) {
//         console.warn('Warning while handling guest wishlist removal', e);
//       }

//       // 🔍 Prevent duplicates in guest cart
//       const rawCart = localStorage.getItem('guestCart');
//       const guestCart = rawCart ? (JSON.parse(rawCart) as Array<{ id: string }>) : [];
//       const existsInCart = guestCart.some((c) => String(c.id) === numericId);

//       if (existsInCart) {
//         // Do NOT re-dispatch, just show warning
//         console.warn('Item already exists in guest cart');
//         return;
//       }

//       await dispatch(addToGuestCart(item)).unwrap();
//       setIsAnimating(true);
